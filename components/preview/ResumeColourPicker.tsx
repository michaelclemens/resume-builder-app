...
export default function ResumeColourPicker({ resume, colourElementRef }: { resume: Resume, colourElementRef: RefObject<HTMLDivElement> }) {
    ...
    const initialBGColour = resume.templateOptions?.colours?.background ?? defaultOptions.background;
    const initialTextColour = resume.templateOptions?.colours?.text ?? defaultOptions.text;
