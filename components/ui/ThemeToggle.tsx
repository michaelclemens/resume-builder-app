  ...
  return (
    <div>
      <label className="group relative z-20 flex cursor-pointer items-center justify-between p-2">
        ...
        {darkMode ? (
          <span className="absolute translate-x-10 text-slate-500 opacity-0 duration-300 ease-in-out dark:opacity-100 group-hover:dark:translate-x-9">
            <FaMoon title={darkModeTitle} />
          </span>
        ) : (
          <span className="absolute translate-x-2 text-orange-400 opacity-100 duration-300 ease-in-out group-hover:translate-x-3 dark:opacity-0">
            <FaSun title={lightModeTitle} />
          </span>
        )}
      </label>
    </div>
