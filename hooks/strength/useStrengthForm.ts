import { StrengthSchema, StrengthSchemaType } from "@/types/strength";
import { addStrength, updateStrength } from "@/lib/client/strength";
import { selectStrengthById, setStrength } from "@/lib/redux/reducers/strength";
import { useAppDispatch, useAppSelector } from "@/lib/redux/store";
import { ResponseStatus } from "@/lib/response";
import { useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";

const useStrengthForm = (strengthId?: string) => {
    const strength = strengthId ? useAppSelector(state => selectStrengthById(state, strengthId)) : null;
    const form = useForm<StrengthSchemaType>({
        resolver: zodResolver(StrengthSchema),
        defaultValues: { name: strength?.name }
    });
    const dispatch = useAppDispatch();
    
    const save = async(resumeId: string, formData: StrengthSchemaType) => {
        const response = strength?.id ? await updateStrength(strength.id, resumeId, formData) : await addStrength(resumeId, formData);

        if (response.status === ResponseStatus.success) {
            dispatch(setStrength(response.payload.strength));
        }

        return response;
    }

    return { strength, save, ...form }
}

export default useStrengthForm;